...
to training
  (py:run "f = h5py.File('modelo 6.h5','r+')")
  (py:run "data_p = f.attrs['training_config']")
  (py:run "data_p = data_p.decode().replace('learning_rate','lr').encode()")
  (py:run "f.attrs['training_config'] = data_p")
  (py:run "f.close()")
  (py:run "model = load_model('modelo 6.h5')")
  (py:run "model.summary()")
  (py:run "print(model.layers[3].get_config())")
  (py:run "print(model.optimizer.lr)")
end
